...
\emergencystretch=3em   % absorb overfull boxes from long \texttt{} tokens/URLs
\usepackage{setspace}
\usepackage{lineno}
\usepackage{fancyhdr}
...
%% ── Title block ─────────────────────────────────────────────
\title{\textbf{CAPA: Directional, Structure-Aware HLA Mismatch Representations
from Protein Language Models for Competing-Risks Survival Analysis in
Haematopoietic Stem Cell Transplantation}\\[6pt]
...
(TRM) preclude one another and must be modelled jointly.  A subtler limitation
affects even the continuous distance metrics proposed as replacements: a
Euclidean mismatch distance is symmetric and therefore \emph{direction-blind},
whereas graft-versus-host alloreactivity is intrinsically directional.  This
work asks whether a representation learned end-to-end can capture directional
mismatch information that no scalar distance can encode.
...
GvHD+TRM risk.  Because the full cross-attention model is under-determined at
the cohort sizes available here, the end-to-end experiments use a compact
469K-parameter variant that self-attends over per-locus \emph{signed}
difference embeddings $\mathbf{e}^{\mathrm{D}}-\mathbf{e}^{\mathrm{R}}$; we
report this variant as CAPA throughout the simulation results.  We evaluate the
framework on the public UCI BMT cohort, a real-world allele-level dataset
(IHWG), and three controlled simulations designed to isolate \emph{what} the
learned representation adds over scalar mismatch distances; we do not claim a
...
models as reference comparators for CAPA.  Because the held-out test set is
small ($n=29$), we treat repeated 5$\times$5 cross-validation as the primary
performance estimate: the cause-specific Cox model reaches relapse
$C = 0.60 \pm 0.14$ and TRM $0.56 \pm 0.06$.  Single held-out-split numbers are
markedly more optimistic (Fine--Gray relapse 0.84, TRM 0.66; Cox 0.75/0.65) but
carry very wide intervals (e.g.\ relapse 95\,\%~CI 0.69--1.00 on four test
events) and should not be over-interpreted; a Random Survival Forest (relapse
0.48) and a flat-feature DeepHit MLP with a correctly formulated censored
log-likelihood (relapse 0.65, TRM 0.57) complete the reference set.  A feature
ablation study shows that aggregate HLA mismatch scores alone yield near-chance
concordance ($C \approx 0.49$), while clinical features alone match the
full-feature model, directly motivating ESM-2 structural allele representations
as the form of HLA information most likely to add discriminative signal.  A
controlled mechanistic simulation provides a capability check: in a cohort where
all patients share an identical binary DRB1 mismatch profile, a Cox model
restricted to binary mismatch achieves GvHD $C = 0.501$ (chance), whereas a Cox
model given the continuous ESM-2 alloreactivity distances achieves $C = 0.695$
($\Delta C = +0.194$)---confirming that a continuous representation recovers a
planted signal invisible to binary indicators, though the signal is generated
from the distances by construction and the experiment is therefore a sanity
check rather than evidence of biological informativeness.  A scale-up
...
signal from raw embeddings without explicit distance features.  Crucially, a
scalar mismatch distance is symmetric and magnitude-only
($\lVert\mathbf{e}^{\mathrm{D}}-\mathbf{e}^{\mathrm{R}}\rVert =
\lVert\mathbf{e}^{\mathrm{R}}-\mathbf{e}^{\mathrm{D}}\rVert$) and is therefore
\emph{blind to the direction} of donor--recipient mismatch, whereas
graft-versus-host alloreactivity is intrinsically directional.  In a directional
GvHD simulation (six seeds), the scalar-distance Cox model collapses to
near-chance ($C_{\mathrm{GvHD}} = 0.575 \pm 0.059$) while CAPA---learning the
immunodominant direction end-to-end from the \emph{signed} difference
embeddings---reaches $C_{\mathrm{GvHD}} = 0.869 \pm 0.023$, a gain of
$\Delta C = +0.294$ that recovers $93\,\%$ of the gap to a direction-aware oracle
($0.892 \pm 0.015$), with non-overlapping confidence intervals on every seed.
This identifies the learned representation's genuine architectural advantage:
it captures directional alloreactivity that no scalar distance can encode.
Real-world
...
(ii)~learns donor--recipient alloreactivity end-to-end via attention over the
\emph{signed} difference embeddings, capturing the \emph{direction} of HLA
mismatch---which antigens the recipient carries that the donor lacks---a
property that the symmetric, magnitude-only scalar distances used in current
practice are structurally incapable of representing; and (iii)~estimates all
three competing risks jointly through a DeepHit survival head
\citep{lee2018deephit} that produces calibrated cumulative incidence functions
over a 730-day post-transplant horizon.
...
direction for future work.  We position this paper accordingly as a
\emph{representation-level proof of concept}: our central, fully supported claim
is that a learned representation captures directional alloreactivity that scalar
mismatch distances provably cannot (Section~\ref{sec:directional}); a
clinically validated outcome predictor would require registry-scale data with
allele-level typing and event-specific endpoints, which we identify but do not
claim to deliver.
...
sequences are retrieved from the IPD-IMGT/HLA database (release 3.57.0)
\citep{barker2023ipdimgt}.  For class~I loci (A, B, C) we
...
and cached in an HDF5 file.  Embedding is a one-time cost that scales with the
size of the allele vocabulary: the 270-allele cache used in this work embeds in
under two minutes on a consumer Apple M-series GPU, while a full registry-scale
vocabulary requires on the order of 4\,h on a single NVIDIA T4 and $\sim$600\,MB
of storage.  The 270-allele cache spanning the five primary loci (and DPB1 when
supplied) is distributed with the open-source release to support architecture
testing.

\paragraph{Per-locus mismatch distance.}
Each individual carries two alleles per locus (one per haplotype), and the
phase pairing of donor to recipient alleles is unknown.  For locus $l$ with
donor allele embeddings $\{\mathbf{e}^{\mathrm{D}}_{l,1},
\mathbf{e}^{\mathrm{D}}_{l,2}\}$ and recipient embeddings
$\{\mathbf{e}^{\mathrm{R}}_{l,1}, \mathbf{e}^{\mathrm{R}}_{l,2}\}$, we resolve
this by optimal assignment, choosing the bijection $\pi$ between the two donor
and two recipient alleles that minimises total Euclidean distance, and define
the scalar per-locus mismatch distance as the mean of the two matched-pair
distances:
\begin{equation}
  d_l = \frac{1}{2}\,
        \min_{\pi \in \{\mathrm{id},\,\mathrm{swap}\}}
        \sum_{m=1}^{2}
        \bigl\lVert \mathbf{e}^{\mathrm{D}}_{l,m}
                   - \mathbf{e}^{\mathrm{R}}_{l,\pi(m)} \bigr\rVert_2 .
  \label{eq:locus_distance}
\end{equation}
The per-locus distances $\{d_l\}$ across the five primary loci constitute the
continuous ESM-2 mismatch features supplied to the distance-based Cox baselines
and to the mechanistic, haploidentical, and directional simulations
(Sections~\ref{sec:mechanistic}--\ref{sec:directional}).  CAPA itself consumes
the full per-allele embeddings (or, in the signed-difference variant, the raw
differences $\mathbf{e}^{\mathrm{D}}_{l,m} - \mathbf{e}^{\mathrm{R}}_{l,m}$)
rather than these scalar summaries, which is precisely what lets it represent
directional information that $d_l$ discards.
...
layers, plus the post-projection MLP) is approximately 27.6\,M, or ${\sim}27.8$\,M
including the DeepHit survival head; this count is dominated by the
cross-attention parameter matrices operating at full embedding dimensionality.
...
\citep{barlow1972isotonic} fitted by \texttt{sklearn.isotonic.\allowbreak IsotonicRegression}
...
\emph{prototype allele cohort}: HLA allele embeddings were computed for 100
commonly occurring alleles drawn from the IPD-IMGT/HLA database (release
3.57.0) spanning loci A, B, C, DRB1, and DQB1.  For figures requiring survival
...
each event $k$, using the competing-risks formulation of
\citet{wolbers2009concordance} with the time-dependent cumulative-incidence
ranking of \citet{antolini2005tdauc}.  A pair $(i,j)$ is \emph{comparable} for
cause $k$ if subject $i$ experiences event $k$ at time $t_i$ and subject $j$ is
still at risk at $t_i$---that is, $t_j > t_i$, irrespective of whether $j$
eventually experiences cause $k$, a competing event, or is censored.  Subjects
who experience a \emph{competing} event before $t_i$ are excluded from $i$'s
comparable set, since their cause-$k$ cumulative incidence can no longer
increase and they are not at risk of being out-ranked.  Formally, with
$\mathcal{P}_k = \{(i,j) : \delta_i = k,\; t_i < t_j\}$,
\begin{equation}
  C_k
    = \frac{
...
incidence for cause $k$ evaluated at the event time of subject $i$.  A value of
0.5 represents chance discrimination; 1.0 is perfect.
...
set of 100 HLA allele embeddings spanning the five primary loci (A, B, C,
...
\begin{figure}[t!]
  ...
  \caption{ESM-2 embeddings of 100 synthetic prototype HLA alleles
    (20 per locus) spanning five primary loci (A, B, C, DRB1, DQB1).
...
prototype allele cohort.  Each panel presents the observed Aalen--Johansen
...
\begin{figure}[t!]
  ...
  \caption{Competing-risks cumulative incidence functions (CIFs) from CAPA
    ...
    One panel per competing event (columns: GvHD, relapse, TRM).
    Within each panel, solid step lines are the observed Aalen--Johansen
    cumulative-incidence estimates \citep{aalen1978empirical} and dashed lines
    are CAPA's mean predicted CIF across patients; shaded bands show the
    interquartile range (25th--75th percentile) of the per-patient predicted
    CIFs.}
  \label{fig:cif}
\end{figure}
...
\subsection{Repeated Cross-Validation: Signed-Difference vs.\ Symmetric Architecture (Imputed Alleles)}
\label{sec:cv_diffmode}

The single-split result above (Section~\ref{sec:capa_imputed}) leaves open
whether CAPA's signed-difference interaction mechanism (Section~\ref{sec:directional})
behaves differently from a symmetric (unsigned) difference representation on
real outcomes, as opposed to the synthetic cohort of
Section~\ref{sec:directional} where the ground-truth hazard is constructed to
depend on direction by design.  We repeated the 5$\times$10 stratified
cross-validation protocol of Section~\ref{sec:cv} on the frequency-imputed
UCI BMT cohort (50 folds total), training CAPA in two configurations ---
self-attention over the symmetric difference $|e^D_l - e^R_l|$, and
self-attention over the signed difference $(e^R_l - e^D_l)$ --- alongside a
fold-matched Cox baseline on scalar per-locus ESM-2 mismatch distance.
GvHD is excluded throughout because no fold retains $\geq 5$ GvHD-only events.

\begin{table}[t!]
  \caption{Repeated 5$\times$10 stratified CV mean $C$-index $\pm$ standard
    deviation across 50 folds, frequency-imputed HLA alleles ($n=187$).
    "Symmetric" and "Signed" are CAPA trained on $|e^D-e^R|$ and $(e^R-e^D)$
    respectively; "Cox (distance)" is a per-fold Cox model on the same scalar
    mismatch distances.}
  \label{tab:cv_diffmode}
  \begin{tabular}{lcc}
    \toprule
    \textbf{Model} & \textbf{Relapse} & \textbf{TRM} \\
    \midrule
    CAPA (symmetric diff)  & $0.516 \pm 0.196$ & $0.522 \pm 0.076$ \\
    CAPA (signed diff)     & $0.495 \pm 0.184$ & $0.523 \pm 0.082$ \\
    Cox (scalar distance)  & $0.462 \pm 0.173$ & $0.607 \pm 0.059$ \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Interpretation.}
The signed and symmetric CAPA variants are statistically indistinguishable on
this cohort ($\Delta C = 0.02$ for relapse, $\Delta C < 0.01$ for TRM, both
far smaller than the $\pm 0.18$--$0.20$ fold-to-fold standard deviation), and
neither CAPA variant outperforms the Cox scalar-distance baseline, which is
in fact better on TRM.  This is the expected outcome given the limitation
established in Section~\ref{sec:capa_imputed}: because imputed alleles carry
no outcome-specific signal, there is no real directional relationship for the
signed-difference mechanism to recover, so this experiment cannot --- and
should not be read to --- test the directional-advantage claim of
Section~\ref{sec:directional}. It instead confirms, under a proper repeated-CV
protocol rather than a single split, that the architectural choice (signed
vs.\ symmetric difference) does not by itself manufacture spurious lift on
data without a real signal, and that demonstrating real directional
advantage requires a cohort with actual donor--recipient allele typing.
...
(95\,\%~CI 0.64--0.75), a gain of $\Delta C = +0.194$ with non-overlapping
confidence intervals.  We emphasise that this simulation generates the GvHD
signal \emph{from} the ESM-2 distances, so the result confirms that the
pipeline recovers a planted continuous signal that a constant binary indicator
cannot---a capability and sanity check---rather than establishing that ESM-2
distances are biologically informative; the latter requires real-data
validation (Section~\ref{sec:ihwg}).  For TRM, which also includes a DRB1 distance
...
In a heterogeneous population with realistic mismatch grade distribution
(0--3 mismatches; 50\,\% matched patients), the aggregate $\Delta C$ is
expected to shrink toward zero: because matched patients (distance\,=\,0) are
correctly handled by both models, the gain from continuous distances is diluted
across the cohort.  This explains why
...
We evaluate CAPA end-to-end on two larger synthetic cohorts where allele-level
information is available by construction.  A note on model capacity: in the
$N=2{,}000$ mechanistic cohort (1{,}600 training patients), the full
bidirectional cross-attention model (${\sim}27.8$M parameters, the default
configuration of Section~\ref{sec:interaction}) did not converge above
Cox(binary)---the optimisation problem of learning donor--recipient
alloreactivity from random initialisation is under-determined at this scale.
The simulations therefore use a compact variant ($d_{\mathrm{proj}}=64$,
469K parameters) that projects the per-locus difference embeddings and
self-attends over them; this is the same architecture trained at two scales,
and we report it as ``CAPA'' throughout the simulation results.  The
haploidentical scale-up (Section~\ref{sec:haplo}, $N=10{,}000$) shows this
variant achieves $C_{\mathrm{GvHD}}=0.732$, statistically indistinguishable from
Cox (ESM-2 distances) at 0.759 (CIs overlap), confirming that the attention
mechanism recovers the main-effect alloreactivity signal from raw embeddings
without explicit distance features.  We caution against the natural expectation
that a \emph{cross-locus interaction} advantage will emerge at registry scale:
the oracle analysis in Section~\ref{sec:haplo} shows that the true multiplicative
interaction term improves concordance by only $+0.001$, because monotone
interactions are rank-redundant for the C-index at any sample size.  The genuine
end-to-end advantage instead appears when the signal is \emph{directional}
(Section~\ref{sec:directional}), a representational property of the learned
model rather than a question of statistical power.
...
A natural hypothesis is that a model able to capture the cross-locus
interaction term $d_{\mathrm{DRB1}}\!\cdot\!d_{\mathrm{DQB1}}$ (coefficient 2.5)
would exceed the main-effects-only Cox model.  We test this directly with an
\emph{oracle} Cox model given the true interaction feature: it reaches
$C_{\mathrm{GvHD}} = 0.760$, a gain of only $+0.001$ over Cox (ESM-2 distances)
at $0.759$ (a model given \emph{all} pairwise products performs identically).
The reason is structural, not a question of sample size: the C-index measures
rank concordance, and a product of two positively-signed distances is nearly
monotone in their sum, so it scarcely reorders patients.  Monotone interactions
are therefore \emph{rank-redundant}, and this ceiling holds at any $N$.  This
result reorients the motivation for the architecture away from interaction
learning and towards the directional signal isolated in
...
Cox (ESM-2 distances) uses main-effect scalars; Cox (dist $+$ true interaction)
is an \emph{oracle} additionally given the true $d_{\mathrm{DRB1}}\!\cdot\!d_{\mathrm{DQB1}}$
term; CAPA (locus attention) self-attends
over per-locus alloreactivity embeddings $|\mathbf{e}^D - \mathbf{e}^R|$ ($d_{\mathrm{proj}}=64$,
469K params).  The oracle's $+0.001$ GvHD gain shows the cross-locus interaction
is rank-redundant for the C-index.
$\Delta_1$ = Cox(distances) $-$ Cox(binary);
$\Delta_{\mathrm{int}}$ = oracle interaction $-$ Cox(distances);
$\Delta_2$ = CAPA $-$ Cox(distances).}
\label{tab:haplo}
\smallskip
\begin{tabular}{lccc}
\toprule
...
$\Delta_{\mathrm{int}}$ (oracle int.\ $-$ dist) & $+0.001$\textsuperscript{ns} & $-0.001$ & $0.000$ \\
...
%% ── Directional Alloreactivity ──────────────────────────────────────────────
\subsection{Directional Alloreactivity: Where the Learned Representation Beats Scalar Distances}
\label{sec:directional}

The haploidentical experiment (Section~\ref{sec:haplo}) shows that CAPA's
self-attention \emph{recovers} the main-effect signal already available to a
Cox model fed hand-computed ESM-2 distances, but does not exceed it.  This is
expected: when the outcome is a monotone function of per-locus alloreactivity
magnitude, a scalar distance $d_l = \lVert \mathbf{e}^{\mathrm{D}}_l -
\mathbf{e}^{\mathrm{R}}_l \rVert$ is a near-sufficient statistic, and no
representation can substantially out-rank it.  We now construct the regime where
this sufficiency \emph{breaks}, and where an end-to-end learned representation
holds a provable advantage over any scalar distance.

\paragraph{Distances are direction-blind by construction.}
A Euclidean distance is symmetric and magnitude-only:
$\lVert \mathbf{e}^{\mathrm{D}}_l - \mathbf{e}^{\mathrm{R}}_l \rVert =
\lVert \mathbf{e}^{\mathrm{R}}_l - \mathbf{e}^{\mathrm{D}}_l \rVert$.
It therefore discards the \emph{direction} of the donor--recipient difference
vector.  Yet alloreactivity is intrinsically directional: graft-versus-host
disease is driven by antigens the \emph{recipient} carries that the
\emph{donor} lacks (donor-derived T~cells recognise them as foreign), which is
encoded by the signed difference $\mathbf{e}^{\mathrm{R}}_l -
\mathbf{e}^{\mathrm{D}}_l$, not by its magnitude.  Two allele pairs with
identical distance can carry opposite immunological meaning depending on which
direction the mismatch points.  A model operating on the \emph{signed}
difference embedding can in principle learn the immunodominant direction; a
scalar distance provably cannot represent it.

\paragraph{Design.}
We generated $N=10{,}000$ all-loci-mismatched patients as in
Section~\ref{sec:haplo}, but drove GvHD hazard by the rectified projection of
the signed per-locus difference onto a fixed, hidden immunodominant direction
$\mathbf{w}_l$ (a seeded unit vector, never revealed to any model):
\begin{align}
\log h_{\mathrm{GvHD}} &= \log\lambda_0^{\mathrm{G}}
    + 2.5\,\mathrm{relu}(z_{\mathrm{DRB1}})
    + 1.5\,\mathrm{relu}(z_{\mathrm{DQB1}})
    + 0.3\,z_{\mathrm{age}}, \\
z_l &= \mathrm{standardise}\big(\mathbf{w}_l \cdot
       (\mathbf{e}^{\mathrm{R}}_l - \mathbf{e}^{\mathrm{D}}_l)\big).
\end{align}
The $\mathrm{relu}(\cdot)$ encodes directionality: only the graft-versus-host
direction raises hazard, while the reverse (host-versus-graft) projection does
not.  As a \emph{control}, TRM was driven by ordinary scalar distances
($2.0\,d_{\mathrm{DRB1}} + 2.0\,d_{\mathrm{A}} + 1.0\,d_{\mathrm{C}}$), a regime
where the distance representation \emph{is} sufficient, and relapse by
$-0.8\,d_{\mathrm{B}}$ plus disease risk.

\paragraph{Models compared.}
We compare four models, all using the same 80/10/10 split:
(i)~\textbf{Cox (binary mismatch)}---all loci mismatched, so GvHD reduces to
age/disease-risk only;
(ii)~\textbf{Cox (scalar distances)}---five per-locus magnitudes $d_l$, the best
a hand-computed distance representation can do;
(iii)~\textbf{Cox (oracle direction)}---additionally given the \emph{true}
directional features $\mathrm{relu}(z_l)$, establishing the achievable ceiling;
and (iv)~\textbf{CAPA (signed diff)}, which self-attends over the raw
\emph{signed} difference embeddings $\mathbf{e}^{\mathrm{D}}_l -
\mathbf{e}^{\mathrm{R}}_l$ (the same 469K-parameter architecture as
Section~\ref{sec:haplo}, trained with $\mathtt{signed\_diff}{=}\mathtt{True}$ so
the directional information is preserved rather than destroyed by an absolute
value).  Results are reported as mean\,$\pm$\,SD over six independent seeds.

\paragraph{Results.}
Table~\ref{tab:directional} reports the outcome.  On GvHD, the scalar-distance
Cox model collapses to near-chance ($C = 0.575 \pm 0.059$): the magnitude of the
mismatch carries little information once the signal lives in its direction.  The
oracle, handed the true directional features, reaches $C = 0.892 \pm 0.015$.
\textbf{CAPA, learning the immunodominant direction end-to-end from the signed
difference embeddings, achieves $C = 0.869 \pm 0.023$}---a gain of
$\Delta C = +0.294$ over the scalar-distance baseline, recovering $93\,\%$ of
the direction-blind gap, with non-overlapping $95\,\%$ confidence intervals on
\emph{every one} of the six seeds.  This is the experiment the haploidentical
setting could not provide: a regime where CAPA does not merely match the
distance baseline but decisively exceeds it, because the learned representation
captures information that is \emph{provably absent} from any scalar distance.

The TRM control confirms the result is specific, not generic.  There the
generating signal is pure scalar distance, the distance-based Cox model is
near-optimal ($C = 0.667 \pm 0.015$, matching its own oracle $0.666$), and
CAPA---tuned end-to-end for the directional GvHD signal---is \emph{weaker}
($C = 0.530 \pm 0.029$).  CAPA wins precisely and only where direction matters;
where magnitude suffices, an explicit distance is the better feature.  The
relapse endpoint, driven by a single scalar distance, is essentially tied
across all models ($C \approx 0.77$--$0.78$), as expected.

\begin{table}[ht]
\centering
\caption{Directional simulation ($N=10{,}000$): test-set C-index
(mean\,$\pm$\,SD over six seeds).  GvHD hazard is driven by the \emph{signed}
projection $\mathbf{w}_l\!\cdot\!(\mathbf{e}^{\mathrm{R}}-\mathbf{e}^{\mathrm{D}})$,
which a symmetric scalar distance cannot represent; TRM is a scalar-distance
control.  Cox (oracle direction) is given the true directional features and
sets the ceiling.  CAPA (signed diff) self-attends over raw signed difference
embeddings.  $\Delta C$ = CAPA $-$ Cox (scalar distances) on GvHD.}
...
Model & GvHD & Relapse & TRM \\
\midrule
Cox (binary mismatch)   & $0.534 \pm 0.024$ & $0.771 \pm 0.006$ & $0.594 \pm 0.025$ \\
Cox (scalar distances)  & $0.575 \pm 0.059$ & $0.782 \pm 0.014$ & $\mathbf{0.667 \pm 0.015}$ \\
Cox (oracle direction)  & $0.892 \pm 0.015$ & $0.782 \pm 0.013$ & $0.666 \pm 0.013$ \\
CAPA (signed diff)      & $\mathbf{0.869 \pm 0.023}$ & $0.769 \pm 0.006$ & $0.530 \pm 0.029$ \\
\midrule
$\Delta C$ (CAPA $-$ distances) & $\mathbf{+0.294}$\textsuperscript{$\ast$} & $-0.013$ & $-0.137$ \\
\bottomrule
\end{tabular}
\smallskip\\
\noindent{\small\textsuperscript{$\ast$} CAPA recovers $93\,\%$ of the
distance-to-oracle gap; the $95\,\%$ bootstrap CIs of CAPA and Cox (scalar
distances) are non-overlapping on all six seeds.  On the TRM control, where the
signal is genuine scalar distance, the distance-based Cox model is near-optimal
and CAPA (tuned for the directional GvHD signal) is weaker---confirming the
advantage is specific to directional structure.}
\end{table}
...
by the specific alleles involved.  The observed advantage is small and, given
the subgroup size ($n = 112$) and the composite OS endpoint, is unlikely to be
statistically distinguishable from zero; we report it as \emph{directionally
consistent} with the mechanistic prediction rather than as independent
confirmation.  Establishing significance would require a bootstrap confidence
interval for this subgroup $\Delta C$ on an event-specific (GvHD) endpoint,
which the composite-OS IHWG data cannot provide; the modest effect should be
...
as 1280-dimensional ESM-2 protein language model vectors, learns directional
donor--recipient alloreactivity end-to-end via attention over the signed
difference embeddings, and jointly estimates
...
It is worth stating precisely \emph{which} property of the architecture
delivers this advantage, because a natural alternative hypothesis---that
attention helps by learning cross-locus interactions---is not supported by our
own experiments.  The haploidentical simulation (Section~\ref{sec:haplo})
includes an oracle Cox model given the \emph{true} cross-locus interaction term
$d_{\mathrm{DRB1}}\!\cdot\!d_{\mathrm{DQB1}}$; it improves concordance by only
$+0.001$ over a main-effects-only model.  This is expected: the C-index measures
rank concordance, and a product of two positively-signed distances is nearly
monotone in their sum, so it barely reorders patients.  Monotone interactions
are therefore \emph{rank-redundant}, and no architecture can win by ``learning
interactions'' in this regime.  The advantage CAPA does hold
(Section~\ref{sec:directional}) is of a different kind: it learns the
\emph{direction} of the donor--recipient difference, information that is absent
from any symmetric scalar distance by construction rather than merely hard to
estimate.  This is the distinction between a representational limitation (which
the learned model overcomes) and a statistical-power limitation (which it does
not).

Beyond prediction, the attention mechanism offers a route to interpretability.
The established primacy of class II mismatching in acute GvHD
...
\citep{fleischhauer2012hla} provide known biology against which a model's
attention weights can be compared.  \emph{Important
...
simulation (Section~\ref{sec:mechanistic}), the haploidentical scale-up
(Section~\ref{sec:haplo}), and the directional simulation
(Section~\ref{sec:directional}) provide empirical evidence that ESM-2 distances
carry the allele-pair information CAPA would exploit and that the learned
representation captures directional signal a scalar distance cannot, but all
three use \emph{simulated} outcomes and are therefore not a substitute for
registry validation.  The directional result in particular should be read as a
\emph{proof of concept}: it establishes that an end-to-end learned
representation can recover directional alloreactivity that is provably outside
the span of scalar distances, but the magnitude of this advantage on real
transplant outcomes---where the directional signal may be weaker, confounded,
or partially captured by other covariates---is unknown.  The directional
advantage also depends on a deliberate architectural choice (feeding the
\emph{signed} rather than absolute difference embedding); and the same model is
\emph{weaker} than an explicit scalar distance on the TRM control, where the
generating signal is pure magnitude.  CAPA should therefore be understood as
advantageous specifically where alloreactivity is directional, not as a
uniform improvement over distance-based scoring.  A second limitation is sample
size: with 187 patients total and 29
...
language model embeddings and learns directional donor--recipient
alloreactivity end-to-end via attention over the signed difference embeddings.  On the public UCI BMT cohort, four tabular-feature
...
The directional simulation (Section~\ref{sec:directional}) isolates the learned
representation's genuine architectural advantage.  A scalar mismatch distance is
symmetric and magnitude-only and is therefore structurally incapable of encoding
the \emph{direction} of donor--recipient mismatch---yet graft-versus-host
alloreactivity is directional, depending on antigens the recipient carries that
the donor lacks.  In a cohort where GvHD hazard is driven by the signed
projection of the difference embedding onto a hidden immunodominant direction,
the scalar-distance Cox model collapses to near-chance ($C_{\mathrm{GvHD}} =
0.575 \pm 0.059$ over six seeds), whereas CAPA---learning that direction
end-to-end from the signed difference embeddings---reaches $0.869 \pm 0.023$,
recovering $93\,\%$ of the gap to a direction-aware oracle with non-overlapping
confidence intervals on every seed.  A TRM control driven by genuine scalar
distance reverses the ordering (distance-based Cox near-optimal, CAPA weaker),
confirming the advantage is specific to directional structure rather than a
generic capacity effect.  This is the clearest statement of what CAPA adds over
the established practice of scalar HLA mismatch scoring: where alloreactivity is
directional, an end-to-end learned representation captures signal that no
hand-computed distance can, while where magnitude alone suffices, an explicit
distance remains the better feature.
...
the directional alloreactivity simulation script (\texttt{scripts/run\_directional\_simulation.py}),
