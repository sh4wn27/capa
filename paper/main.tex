...
combinations of these distances from data.  In the $N=2{,}000$ mechanistic
cohort (1{,}600 training patients), the full cross-attention model
(${\sim}27$M parameters) did not converge above Cox(binary).  The haploidentical
scale-up simulation (Section~\ref{sec:haplo}) scales to $N=10{,}000$ and
uses a compact variant ($d_{\mathrm{proj}}=64$, 469K parameters) that self-attends
over per-locus alloreactivity embeddings: CAPA achieves $C_{\mathrm{GvHD}}=0.732$,
statistically indistinguishable from Cox (ESM-2 distances) at 0.759 (CIs overlap),
confirming that the attention mechanism recovers the main-effect alloreactivity
signal from raw embeddings without explicit distance features.  The cross-locus
interaction advantage is expected to emerge at registry scale ($N \gg 10{,}000$),
where the signal-to-noise ratio for the multiplicative interaction terms
($d_{\mathrm{DRB1}}\!\cdot\!d_{\mathrm{DQB1}}$, coefficient 2.5) reaches
sufficient power.
