...
\url{https://github.com/sh4wn27/capa?tab=readme-ov-file} under the MIT licence.
...
state at sequence position $i$.  ESM-2 weights are kept \emph{fully frozen};
no transplantation-specific fine-tuning is performed.  Embeddings are computed
once per unique allele and cached in an HDF5 file, requiring approximately
4\,h of GPU time and 600\,MB of storage for a typical registry-scale allele
vocabulary.  A prototype cache of 98 alleles spanning all five loci is
...
set of 98 HLA allele embeddings spanning all five loci (A, B, C, DRB1, DQB1),
...
Fine-tuning ESM-2 on HLA-specific or immunology-specific protein sequences
is a promising direction for improving embedding quality beyond what the
general-purpose UR50D training achieves.  Incorporating linkage disequilibrium
structure across loci---rather than treating loci as independent in the
embedding step---is another architectural extension worth exploring.
...
the interactive web interface, and model weights trained on synthetic allele data
are released under the MIT licence to lower the barrier to this next step.
