...
correcting for the optimism in the single 29-patient test split.  The dataset
does not contain per-allele HLA typing strings; full validation of CAPA's
ESM-2 embedding pipeline requires a registry dataset with high-resolution
allele-level data, which we identify as the primary direction for future work.
...
To validate the full CAPA pipeline on real clinical outcomes, we performed a
controlled end-to-end experiment: donor--recipient allele pairs were imputed
from European population frequency tables subject to each patient's recorded
mismatch count, and CAPA was trained end-to-end (ESM-2 embeddings $\to$
cross-attention $\to$ DeepHit) on the resulting dataset.  As expected, randomly
assigned alleles carry no outcome signal ($C=0.43$ relapse, $C=0.52$ TRM),
confirming both that the architectural pipeline executes correctly and that real
allele-level typing data is the essential requirement for CAPA to provide
predictive value beyond clinical features alone.
...
and the end-to-end CAPA evaluation script (\texttt{scripts/run\_capa\_imputed.py})
